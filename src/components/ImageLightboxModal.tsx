...
export const ImageLightboxModal: React.FC<ImageLightboxModalProps> = ({ selectedItem, onClose }) => {
  const focusTrapRef = useFocusTrap<HTMLDivElement>({ isActive: Boolean(selectedItem), onEscape: onClose });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <img
          {...getResponsiveImageProps(selectedItem.image, '(min-width: 1024px) 960px, 100vw', [640, 960, 1280, 1600])}
        ...
        <div className="p-4 bg-[#0b0b0b] border-t border-gray-800 text-center">
          ...
          <h3 id="image-lightbox-title" className="font-serif text-lg font-bold text-white">
            {selectedItem.title}
          </h3>
        </div>
      </div>
